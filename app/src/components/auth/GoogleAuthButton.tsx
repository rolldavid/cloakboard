...
export function GoogleAuthButton({ disabled = false, className = '' }: GoogleAuthButtonProps) {
  ...
  const handleClick = () => {
    if (!isConfigured) {
      alert('Google OAuth not configured. Set VITE_GOOGLE_CLIENT_ID.');
      return;
    }
  ...
  return (
    <button
      ...
      className={`flex items-center justify-center gap-3 w-full p-4 rounded-lg border-2 border-accent bg-accent-muted hover:bg-accent-muted transition-all text-left disabled:opacity-50 ${className}`}
    >
