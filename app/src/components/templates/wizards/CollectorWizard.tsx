...
const STEPS: WizardStepType<StepId>[] = [
  {
    ...
    label: 'Basics',
    validate: (config) => {
      const c = config as CollectorConfig;
      if (!c.name?.trim()) return 'Collective name is required';
  ...
  { id: 'privacy', label: 'Privacy' },
  {
    ...
    validate: (config) => {
      ...
      if (c.approvalThreshold < 50 || c.approvalThreshold > 100)
        return 'Approval threshold must be 50-100%';
      return null;
    },
  },
...
interface CollectorWizardProps {
  onSubmit: (config: CollectorConfig) => Promise<void>;
}

export function CollectorWizard({ onSubmit }: CollectorWizardProps) {
  const initialConfig: CollectorConfig = {
    name: '',
    description: '',
    votingDuration: 72000, // ~5 days
    quorumThreshold: 3,
    approvalThreshold: 66, // 66%
    minContribution: BigInt('10000000000000000'), // 0.01 ETH
  ...
  const wizard = useWizard<StepId, CollectorConfig>({
    steps: STEPS,
    initialConfig,
    storageKey: 'collector-cloak-draft',
    onComplete: onSubmit,
  });

  const votingDays = Math.round(wizard.config.votingDuration / (24 * 60 * 10));
  const minContributionEth = Number(wizard.config.minContribution) / 1e18;
  ...
  return (
    <BaseWizard
      ...
      submitLabel="Create Collector Cloak"
      cancelPath="/create"
    >
      {wizard.currentStep === 'basics' && (
        <WizardStep title="Collective Details" description="Basic information about your collector Cloak">
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g., Digital Art Collective"
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Describe your collection focus (e.g., generative art, photography, historical NFTs)..."
                rows={3}
                className="w-full px-4 py-2 border border-border rounded-md focus:ring-2 focus:ring-ring focus:border-ring"
              />
            </div>
      ...
      {wizard.currentStep === 'capital' && (
        <WizardStep title="Capital Requirements" description="Set contribution terms">
          <div className="space-y-6">
            <div>
              ...
              <input
                ...
                max={1}
                step={0.001}
                value={minContributionEth}
                onChange={(e) =>
                  wizard.updateConfig({
                    minContribution: BigInt(Math.floor(parseFloat(e.target.value) * 1e18)),
                  })
                }
                className="w-full"
              />
              ...
              <p className="text-xs text-foreground-muted mt-1">
                Minimum capital each member must contribute
              </p>
            </div>

            <div className="p-4 bg-template-pink/10 border border-template-pink/20 rounded-md">
              <h4 className="font-medium text-template-pink mb-2">Collection Ownership</h4>
              <p className="text-sm text-template-pink">
                Ownership of acquired pieces is proportional to member contributions. When a piece
                is sold, proceeds are distributed to members based on their share.
              </p>
            </div>
          </div>
        </WizardStep>
      ...
      {wizard.currentStep === 'privacy' && (
        <WizardStep
          ...
          description="Choose visibility of collection operations"
        >
          <PrivacySlider
      ...
      {wizard.currentStep === 'governance' && (
        <WizardStep title="Governance Settings" description="Configure acquisition voting">
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-foreground-secondary mb-1">
            ...
            <div>
              <label className="block text-sm font-medium text-foreground-secondary mb-1">
                Approval Threshold: {wizard.config.approvalThreshold}%
              </label>
              <input
                type="range"
                min={50}
                max={100}
                step={1}
                value={wizard.config.approvalThreshold}
                onChange={(e) =>
                  wizard.updateConfig({ approvalThreshold: parseInt(e.target.value) })
                }
                className="w-full"
              />
              <div className="flex justify-between text-xs text-foreground-muted">
              ...
              <p className="text-xs text-foreground-muted mt-1">
                Percentage of votes needed to approve an acquisition
              </p>
            </div>
            ...
            <div>
              ...
              />
              <p className="text-xs text-foreground-muted mt-1">
                Minimum number of members who must vote
              </p>
            </div>
          </div>
        </WizardStep>
      ...
      {wizard.currentStep === 'review' && (
        <WizardStep title="Review & Create" description="Confirm your collective settings">
          <div className="space-y-4">
            ...
            <div className="p-4 bg-background-secondary rounded-md">
              ...
              <p className="text-foreground">Min contribution: {minContributionEth} ETH</p>
            </div>
            ...
            <div className="p-4 bg-background-secondary rounded-md">
              ...
              <p className="text-foreground-secondary text-sm">
                Approval: {wizard.config.approvalThreshold}%
              </p>
