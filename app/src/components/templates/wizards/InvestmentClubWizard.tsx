...
type StepId = 'basics' | 'membership' | 'capital' | 'privacy' | 'governance' | 'review';

interface InvestmentClubConfig {
  name: string;
  description: string;
...
const STEPS: WizardStepType<StepId>[] = [
  {
    ...
    label: 'Basics',
    validate: (config) => {
      const c = config as InvestmentClubConfig;
      if (!c.name?.trim()) return 'Club name is required';
  ...
  },
  {
    ...
    validate: (config) => {
      const c = config as InvestmentClubConfig;
      if (c.maxMembers < 2) return 'Must allow at least 2 members';
      return null;
    },
  },
...
interface InvestmentClubWizardProps {
  onSubmit: (config: InvestmentClubConfig) => Promise<void>;
}

export function InvestmentClubWizard({ onSubmit }: InvestmentClubWizardProps) {
  const initialConfig: InvestmentClubConfig = {
    name: '',
    description: '',
    votingDuration: 72000, // ~5 days
    quorumThreshold: 3,
    minCommitment: BigInt('100000000000000000'), // 0.1 ETH
    maxMembers: 99,
  ...
  const wizard = useWizard<StepId, InvestmentClubConfig>({
    steps: STEPS,
    initialConfig,
    storageKey: 'investment-club-draft',
    onComplete: onSubmit,
  });

  const votingDays = Math.round(wizard.config.votingDuration / (24 * 60 * 10));
  const minCommitmentEth = Number(wizard.config.minCommitment) / 1e18;
  ...
  return (
    <BaseWizard
      ...
      submitLabel="Create Investment Club"
      cancelPath="/create"
    >
      {wizard.currentStep === 'basics' && (
        <WizardStep title="Club Details" description="Basic information about your investment club">
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-foreground-secondary mb-1">Club Name *</label>
              <input
                type="text"
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Describe the investment thesis and focus..."
                rows={3}
                className="w-full px-4 py-2 border border-border rounded-md focus:ring-2 focus:ring-ring focus:border-ring"
              />
            </div>
      ...
      {wizard.currentStep === 'capital' && (
        <WizardStep title="Capital Requirements" description="Set membership and contribution terms">
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-foreground-secondary mb-1">
                Minimum Commitment: {minCommitmentEth} ETH
              </label>
              <input
                type="range"
                min={0.01}
                max={10}
                step={0.01}
                value={minCommitmentEth}
                onChange={(e) =>
                  wizard.updateConfig({
                    minCommitment: BigInt(Math.floor(parseFloat(e.target.value) * 1e18)),
                  })
                }
                className="w-full"
              />
              ...
              <p className="text-xs text-foreground-muted mt-1">Minimum capital each member must commit</p>
            </div>

            <div>
              <label className="block text-sm font-medium text-foreground-secondary mb-1">Maximum Members</label>
              <input
                type="number"
                min={2}
                max={99}
                value={wizard.config.maxMembers}
                onChange={(e) =>
                  wizard.updateConfig({ maxMembers: parseInt(e.target.value) || 2 })
                }
                className="w-full px-4 py-2 border border-border rounded-md focus:ring-2 focus:ring-ring focus:border-ring"
              />
              <p className="text-xs text-foreground-muted mt-1">
                Limited to 99 to comply with investment club regulations
              </p>
            </div>

            <div className="p-4 bg-status-info/10 border border-status-info/20 rounded-md">
              <p className="text-sm text-status-info">
                <strong>Note:</strong> Voting power is proportional to capital contribution. Members
                with larger commitments have more voting power on investment decisions.
              </p>
            </div>
          </div>
        </WizardStep>
      ...
      {wizard.currentStep === 'privacy' && (
        <WizardStep
          ...
          description="Choose visibility of club operations"
        >
          <PrivacySlider
      ...
      {wizard.currentStep === 'governance' && (
        <WizardStep title="Governance Settings" description="Configure deal voting">
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-foreground-secondary mb-1">
            ...
            <div>
              ...
              />
              <p className="text-xs text-foreground-muted mt-1">
                Minimum number of members who must vote on deals
              </p>
            </div>
          </div>
        </WizardStep>
      ...
      {wizard.currentStep === 'review' && (
        <WizardStep title="Review & Create" description="Confirm your club settings">
          <div className="space-y-4">
            ...
            <div className="p-4 bg-background-secondary rounded-md">
              ...
              <p className="text-foreground">Min commitment: {minCommitmentEth} ETH</p>
              <p className="text-foreground-secondary text-sm">Max members: {wizard.config.maxMembers}</p>
            </div>
            ...
            <div className="p-4 bg-background-secondary rounded-md">
              ...
              <p className="text-foreground-secondary text-sm">Quorum: {wizard.config.quorumThreshold} members</p>
            </div>
          </div>
