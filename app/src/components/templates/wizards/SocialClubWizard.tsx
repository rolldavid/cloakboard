...
const STEPS: WizardStepType<StepId>[] = [
  {
    ...
    label: 'Basics',
    validate: (config) => {
      const c = config as SocialClubConfig;
...
interface SocialClubWizardProps {
  onSubmit: (config: SocialClubConfig) => Promise<void>;
}

export function SocialClubWizard({ onSubmit }: SocialClubWizardProps) {
  const initialConfig: SocialClubConfig = {
    name: '',
    description: '',
    votingDuration: 72000, // ~5 days
    quorumThreshold: 5,
    basicDues: BigInt('10000000000000000'), // 0.01 ETH
  ...
  const wizard = useWizard<StepId, SocialClubConfig>({
    steps: STEPS,
    initialConfig,
    storageKey: 'social-club-draft',
    onComplete: onSubmit,
  });

  const votingDays = Math.round(wizard.config.votingDuration / (24 * 60 * 10));
  const basicDuesEth = Number(wizard.config.basicDues) / 1e18;
  const premiumDuesEth = Number(wizard.config.premiumDues) / 1e18;
  ...
  return (
    <BaseWizard
      ...
      submitLabel="Create Social Club"
      cancelPath="/create"
    >
      {wizard.currentStep === 'basics' && (
        <WizardStep title="Club Details" description="Basic information about your social club">
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g., The Founders Club"
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Describe your community and what brings members together..."
                rows={3}
                className="w-full px-4 py-2 border border-border rounded-md focus:ring-2 focus:ring-ring focus:border-ring"
              />
            </div>
      ...
      {wizard.currentStep === 'membership' && (
        <WizardStep title="Membership Tiers" description="Configure dues and membership levels">
          ...
          <div className="space-y-6 mt-6">
            <div>
              <label className="block text-sm font-medium text-foreground-secondary mb-1">
                Basic Dues: {basicDuesEth} ETH / month
              </label>
              <input
                type="range"
                min={0.001}
                ...
                onChange={(e) =>
                  wizard.updateConfig({
                    basicDues: BigInt(Math.floor(parseFloat(e.target.value) * 1e18)),
                  })
                }
                className="w-full"
              />
              ...
              <p className="text-xs text-foreground-muted mt-1">Standard membership tier</p>
            </div>

            <div>
              <label className="block text-sm font-medium text-foreground-secondary mb-1">
                Premium Dues: {premiumDuesEth} ETH / month
              </label>
              <input
                type="range"
                min={0.01}
                max={1}
                step={0.01}
                value={premiumDuesEth}
                onChange={(e) =>
                  wizard.updateConfig({
                    premiumDues: BigInt(Math.floor(parseFloat(e.target.value) * 1e18)),
                  })
                }
                className="w-full"
              />
              ...
              <p className="text-xs text-foreground-muted mt-1">Premium tier with extra perks</p>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 bg-background-secondary border border-border rounded-md">
                <h4 className="font-medium text-foreground mb-2">Basic Tier</h4>
              ...
              <div className="p-4 bg-template-violet/10 border border-template-violet/20 rounded-md">
                <h4 className="font-medium text-template-violet mb-2">Premium Tier</h4>
                <ul className="text-sm text-template-violet space-y-1">
      ...
      {wizard.currentStep === 'governance' && (
        <WizardStep title="Governance Settings" description="Configure club decisions">
          <div className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2 border border-border rounded-md focus:ring-2 focus:ring-ring focus:border-ring"
              />
              <p className="text-xs text-foreground-muted mt-1">Minimum votes needed for decisions</p>
            </div>
          </div>
        </WizardStep>
      )}
        ...
        <WizardStep title="Review & Create" description="Confirm your club settings">
          <div className="space-y-4">
            ...
            <div className="p-4 bg-background-secondary rounded-md">
              ...
              <p className="text-foreground">Basic: {basicDuesEth} ETH/month</p>
              <p className="text-foreground-secondary text-sm">Premium: {premiumDuesEth} ETH/month</p>
            </div>
