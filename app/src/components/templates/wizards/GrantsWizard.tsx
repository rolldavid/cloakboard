...
type StepId = 'basics' | 'membership' | 'review_process' | 'privacy' | 'review';
...
const STEPS: WizardStepType<StepId>[] = [
  {
    ...
    label: 'Basics',
    validate: (config) => {
      const c = config as GrantsConfig;
      if (!c.name?.trim()) return 'Program name is required';
  ...
  },
  {
    ...
    validate: (config) => {
      const c = config as GrantsConfig;
      if (c.minReviewers < 1) return 'At least 1 reviewer required';
...
interface GrantsWizardProps {
  onSubmit: (config: GrantsConfig) => Promise<void>;
}

export function GrantsWizard({ onSubmit }: GrantsWizardProps) {
  ...
  const wizard = useWizard<StepId, GrantsConfig>({
    steps: STEPS,
    initialConfig,
    storageKey: 'grants-cloak-draft',
    onComplete: onSubmit,
  });
  ...
  return (
    <BaseWizard
      ...
      submitLabel="Create Grants Program"
      cancelPath="/create"
    >
      {wizard.currentStep === 'basics' && (
        <WizardStep title="Program Details" description="Basic information about your grants program">
          <div className="space-y-4">
            <div>
              <CloakNameInput
                value={wizard.config.name}
                onChange={(name) => wizard.updateConfig({ name })}
                placeholder="e.g., Ecosystem Grants"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Describe what types of projects this grants program funds..."
                rows={3}
      ...
      {wizard.currentStep === 'review_process' && (
        <WizardStep title="Review Process" description="Configure application review settings">
          <div className="space-y-6">
            <div>
              ...
              <input
                ...
                min={1}
                max={10}
                value={wizard.config.minReviewers}
                onChange={(e) =>
                  wizard.updateConfig({ minReviewers: parseInt(e.target.value) || 1 })
                }
                className="w-full px-4 py-2 border border-border rounded-md focus:ring-2 focus:ring-ring focus:border-ring"
              />
              <p className="text-xs text-foreground-muted mt-1">
                Number of reviewers needed before an application can proceed to vote
              </p>
            </div>
            ...
            <div>
              ...
              />
              <p className="text-xs text-foreground-muted mt-1">
                Minimum votes needed for a grant to be approved
              </p>
            </div>

            <div className="p-4 bg-template-purple/10 border border-template-purple/20 rounded-md">
              <h4 className="font-medium text-template-purple mb-2">Grant Workflow</h4>
              <ol className="list-decimal list-inside text-sm text-template-purple space-y-1">
                <li>Applicant submits proposal</li>
                <li>Reviewers evaluate and score (min {wizard.config.minReviewers} reviews)</li>
                <li>Committee votes on funding</li>
                <li>If approved, milestone-based funding begins</li>
              </ol>
            </div>
          </div>
        </WizardStep>
      ...
      {wizard.currentStep === 'privacy' && (
        <WizardStep
          ...
          description="Choose visibility of program operations"
        >
          <PrivacySlider
      ...
      {wizard.currentStep === 'review' && (
        <WizardStep title="Review & Create" description="Confirm your program settings">
          <div className="space-y-4">
            ...
            <div className="p-4 bg-background-secondary rounded-md">
              ...
              <p className="text-foreground">Min reviewers: {wizard.config.minReviewers}</p>
              <p className="text-foreground-secondary text-sm">Voting: {votingDays} days</p>
