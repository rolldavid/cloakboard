#!/usr/bin/env node
/**
 * DuelCloak V7 Deployment
 *
 * Reuses existing keeper wallet, FPC, and account class.
 * Publishes the new V7 DuelCloak contract class and deploys a new instance.
 *
 * V7 changes:
 * - submit_and_start_duel: removed sequential duel constraint (concurrent duels)
 * - submit_and_start_duel: added end_block param (server-computed per-duel)
 *
 * Also copies the compiled artifact to server/src/lib/aztec/artifacts/ and
 * app/src/lib/aztec/artifacts/ for runtime use.
 *
 * Usage: cd contracts && npx tsx scripts/deploy-v7.ts
 */
...
async function main() {
  ...
  console.log('DuelCloak V7 Deployment');
  console.log('='.repeat(60));
  ...
  // Step 3: Publish V7 DuelCloak class
  console.log('[3/5] Publishing V7 DuelCloak class...');
  ...
  if (published) {
    console.log('  V7 class already published');
  } else {
    try {
  ...
  // Step 4: Deploy V7 DuelCloak instance
  console.log('[4/5] Deploying V7 DuelCloak instance...');
  ...
  try {
    ...
    console.log(`  V7 DuelCloak deployed at: ${duelCloakAddress}`);
  } catch (deployErr: any) {
    ...
    if (onChain) {
      ...
      console.log(`  V7 DuelCloak deployed at: ${duelCloakAddress} (verified on-chain)`);
    } else {
      throw deployErr;
    }
  }
