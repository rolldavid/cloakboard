#!/usr/bin/env node
/**
 * Fresh Dev Deployment — All Contracts
 *
 * Reuses existing keeper + SponsoredFPC, deploys fresh instances of:
 * 1. UserProfile
 * 2. DuelCloak (linked to UserProfile)
 * 3. VoteHistory
 * 4. LinkRegistry
 *
 * Usage: cd contracts && npx tsx scripts/deploy-dev-fresh.ts
 */
...
if (!KEEPER_SECRET_KEY || !KEEPER_SIGNING_KEY || !KEEPER_SALT || !KEEPER_ADDRESS) {
  console.error('Missing keeper keys in server/.env.local');
  process.exit(1);
}
if (!FPC_ADDRESS) {
  console.error('Missing VITE_SPONSORED_FPC_ADDRESS in server/.env.local');
  process.exit(1);
}
...
async function deployContract(
  ...
  args: any[],
  label: string,
  keeperAddress: AztecAddress,
  sponsoredPayment: any,
  node: any,
) {
  const deploy = Contract.deploy(patchedWallet, artifact, args);
  ...
  try {
    ...
    const addr = deployed.address.toString();
    ...
    if (onChain) {
      ...
      console.log(`  ${label} deployed at: ${addr} (verified on-chain)`);
      return addr;
    }
    throw deployErr;
  }
...
async function main() {
  ...
  console.log('Fresh Dev Deployment — All Contracts');
  console.log('='.repeat(60));
  ...
  try {
    ...
    // Fallback: register from canonical params
  ...
  // Step 3: Deploy UserProfile
  console.log('[3/9] Publishing & deploying UserProfile...');
  ...
  const userProfileClassId = await publishClass(patchedWallet, userProfileArtifact, 'UserProfile', keeperAddress, sponsoredPayment);
  const userProfileAddress = await deployContract(patchedWallet, userProfileArtifact, [keeperAddress], 'UserProfile', keeperAddress, sponsoredPayment, node);

  // Step 4: Deploy DuelCloak
  console.log('[4/9] Publishing & deploying DuelCloak...');
  ...
  const duelCloakClassId = await publishClass(patchedWallet, duelCloakArtifact, 'DuelCloak', keeperAddress, sponsoredPayment);
  ...
  const duelCloakAddress = await deployContract(patchedWallet, duelCloakArtifact, constructorArgs, 'DuelCloak', keeperAddress, sponsoredPayment, node);

  // Step 5: Deploy VoteHistory
  console.log('[5/9] Publishing & deploying VoteHistory...');
  ...
  await publishClass(patchedWallet, voteHistoryArtifact, 'VoteHistory', keeperAddress, sponsoredPayment);
  const voteHistoryAddress = await deployContract(patchedWallet, voteHistoryArtifact, [], 'VoteHistory', keeperAddress, sponsoredPayment, node);

  // Step 6: Deploy LinkRegistry
  console.log('[6/9] Publishing & deploying LinkRegistry...');
  ...
  await publishClass(patchedWallet, linkRegistryArtifact, 'LinkRegistry', keeperAddress, sponsoredPayment);
  const linkRegistryAddress = await deployContract(patchedWallet, linkRegistryArtifact, [], 'LinkRegistry', keeperAddress, sponsoredPayment, node);

  // Step 7: Link contracts
  console.log('[7/9] Linking contracts: set_authorized_caller...');
  ...
  console.log('[8/9] Copying artifacts...');
  const artifactCopies: [string, string][] = [
  ...
  console.log('[9/9] Updating env files...');
  const envFiles = [
  ...
  console.log('');
  console.log('='.repeat(60));
  console.log('FRESH DEV DEPLOYMENT COMPLETE!');
  console.log('='.repeat(60));
  ...
  console.log('  1. Set VITE_CREATE_DUEL_THRESHOLD=0 for bootstrapping');
