#!/usr/bin/env node
/**
 * DuelCloak V8 + UserProfile V7 Combined Deployment
 *
 * Deploy order:
 * 1. Deploy UserProfile V7 with constructor(keeperAddress) -- initializes staking params
 * 2. Deploy DuelCloak V8 with constructor(..., userProfileV7Address)
 * 3. Call UserProfile.set_authorized_caller(duelCloakV8Address) via keeper -- links them
 *
 * V7 changes from V6:
 * - New staking storage: stakes, total_staked, total_burned, min_stake, min_votes_threshold
 * - New functions: stake_points, resolve_stake, burn_stake, update_staking_params
 * - Constructor now initializes min_stake=100, min_votes_threshold=10
 *
 ...
 *
 * Usage: cd contracts && npx tsx scripts/deploy-v9-combined.ts
 */
...
async function main() {
  ...
  console.log('DuelCloak V8 + UserProfile V7 Combined Deployment');
  console.log('='.repeat(60));
  ...
  // Step 3: Publish UserProfile V7 class
  console.log('[3/8] Publishing UserProfile V7 class...');
  ...
  const userProfileClassId = await publishClass(patchedWallet, userProfileArtifact, 'UserProfile V7', keeperAddress, sponsoredPayment);

  // Step 4: Deploy UserProfile V7 instance
  console.log('[4/8] Deploying UserProfile V7 instance...');
  ...
  console.log('');
  console.log('='.repeat(60));
  console.log('V9 COMBINED DEPLOYMENT COMPLETE!');
  console.log('  (DuelCloak V8 unchanged + UserProfile V7 with staking)');
  console.log('='.repeat(60));
  ...
  console.log(`  UserProfile V7 Class ID: ${userProfileClassId}`);
  console.log(`  KEEPER_ADDRESS=${KEEPER_ADDRESS}`);
  console.log(`  VITE_SPONSORED_FPC_ADDRESS=${FPC_ADDRESS}`);
  console.log('');
  console.log('Both contracts linked: DuelCloak V8 -> UserProfile V7 (authorized_caller)');
  ...
  console.log('  3. Run V17 migration on server for queue DB schema');
}
