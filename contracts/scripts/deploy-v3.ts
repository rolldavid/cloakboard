#!/usr/bin/env node
/**
 * DuelCloak V3 Upgrade Deployment
 *
 * Reuses existing keeper wallet, FPC, and MultiAuth class.
 * Only publishes the new V3 DuelCloak contract class and deploys a new instance.
 *
 * Usage: cd contracts && npx tsx scripts/deploy-v3.ts
 */
...
async function main() {
  ...
  console.log('DuelCloak V3 Upgrade Deployment');
  console.log('='.repeat(60));
  ...
  // Publish new V3 DuelCloak class
  console.log('[3/4] Publishing V3 DuelCloak class...');
  ...
  if (published) {
    console.log('  V3 class already published');
  } else {
    try {
  ...
  // Deploy new V3 DuelCloak instance
  console.log('[4/4] Deploying V3 DuelCloak instance...');
  const constructorArgs = [
    ...
    0,                      // _tally_mode: u8 (unused in V3, backward compat)
  ...
  try {
    ...
    console.log(`  V3 DuelCloak deployed at: ${duelCloakAddress}`);
  } catch (deployErr: any) {
    ...
    if (onChain) {
      ...
      console.log(`  V3 DuelCloak deployed at: ${duelCloakAddress} (verified on-chain)`);
    } else {
      throw deployErr;
    }
  }
  ...
  // Update server/.env.local - just replace VITE_DUELCLOAK_ADDRESS
